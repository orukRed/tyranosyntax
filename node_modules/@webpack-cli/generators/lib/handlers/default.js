...
const templatePath = path_1.default.resolve(__dirname, "../../init-template/default");
const resolveFile = (file) => {
    return path_1.default.resolve(templatePath, file);
};
async function questions(self, Question, config = {
    langType: {},
    devServer: {},
    htmlWebpackPlugin: {},
    ...
    const { langType } = await Question.List(self, "langType", "Which of the following JS solutions do you want to use?", [config.langType.required ? "" : "none", "ES6", "Typescript"].filter((option) => option.length > 0), config.langType.required ? "ES6" : "none", self.force || config.langType.skip);
    switch (langType) {
        case "ES6":
    ...
    const { devServer } = await Question.Confirm(self, "devServer", "Do you want to use webpack-dev-server?", true, self.force || config.devServer.skip);
    if (devServer) {
        self.dependencies.push("webpack-dev-server");
    }
    // Handle addition of html-webpack-plugin
    const { htmlWebpackPlugin } = await Question.Confirm(self, "htmlWebpackPlugin", "Do you want to simplify the creation of HTML files for your bundle?", true, self.force || config.htmlWebpackPlugin.skip);
    if (htmlWebpackPlugin) {
        self.dependencies.push("html-webpack-plugin");
    }
    // Handle addition of workbox-webpack-plugin
    const { workboxWebpackPlugin } = await Question.Confirm(self, "workboxWebpackPlugin", "Do you want to add PWA support?", true, self.force || config.workboxWebpackPlugin.skip);
    if (workboxWebpackPlugin) {
        self.dependencies.push("workbox-webpack-plugin");
    }
    // Store all answers for generation
    self.answers = Object.assign(Object.assign({}, self.answers), { langType,
        devServer,
        htmlWebpackPlugin,
        workboxWebpackPlugin });
    // Handle CSS solutions
    const { cssType } = await Question.List(self, "cssType", "Which of the following CSS solutions do you want to use?", [config.cssType.required ? "" : "none", "CSS only", "SASS", "LESS", "Stylus"].filter((option) => option.length > 0), config.cssType.required ? "CSS only" : "none", self.force || config.cssType.skip);
    if (cssType == "none") {
        self.answers = Object.assign(Object.assign({}, self.answers), { cssType, isCSS: false, isPostCSS: false, extractPlugin: "No" });
        return;
    }
    const { isCSS } = cssType != "CSS only"
    ...
    self.answers = Object.assign(Object.assign({}, self.answers), { cssType,
        isCSS,
        isPostCSS,
        extractPlugin });
}
...
function generate(self) {
    self.fs.extendJSON(self.destinationPath("package.json"), 
    ...
    // Generate HTML file
    self.fs.copyTpl(resolveFile("template.html.tpl"), self.destinationPath("index.html"), self.answers);
    // Generate webpack configuration
    self.fs.copyTpl(resolveFile("webpack.configjs.tpl"), self.destinationPath("webpack.config.js"), Object.assign(Object.assign({}, self.answers), { entry }));
