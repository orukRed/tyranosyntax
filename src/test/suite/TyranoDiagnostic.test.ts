...
suite('TyranoDiagnostic.isValueIsVariable', () => {
	...
	test('f.hoge true返却', () => {
		...
		assert.strictEqual((diag as any).isValueIsVariable("f.hoge"), true);
	});
	test('sf.hoge true返却', () => {
		//値定義
		const diag = new TyranoDiagnostic();
		assert.strictEqual((diag as any).isValueIsVariable("sf.hoge"), true);
	});
	test('tf.hoge true返却', () => {
		//値定義
		const diag = new TyranoDiagnostic();
		assert.strictEqual((diag as any).isValueIsVariable("tf.hoge"), true);
	});
	test('mp.hoge true返却', () => {
		//値定義
		const diag = new TyranoDiagnostic();
		assert.strictEqual((diag as any).isValueIsVariable("mp.hoge"), true);
	});
	...
	test('hoge false返却', () => {
		//値定義
		const diag = new TyranoDiagnostic();
		assert.strictEqual((diag as any).isValueIsVariable("hoge"), false);
	});
	...
	test('a&hoge false返却', () => {
		//値定義
		const diag = new TyranoDiagnostic();
		assert.strictEqual((diag as any).isValueIsVariable("a&hoge"), false);
	});
