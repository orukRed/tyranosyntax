...
suite("InformationWorkSpace.getInstance", () => {
  test("正常系 シングルトンパターン", () => {
    ...
    assert.ok(instance1 instanceof InformationWorkSpace, "InformationWorkSpaceのインスタンスであるべき");
  });
});
...
suite("InformationWorkSpace.getProjectPathByFilePath additional tests", () => {
  test("正常系 独立テスト", async () => {
    ...
    try {
      ...
      // 戻り値は文字列またはundefined
      assert.ok(typeof result === "string" || result === undefined, "戻り値は文字列またはundefinedであるべき");
    } catch (error) {
      // エラーは想定内
