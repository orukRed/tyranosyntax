...
suite("InformationExtension", () => {
  ...
  suite("getInstance", () => {
    test("正常系 インスタンスが取得できる", () => {
      ...
      assert.ok(instance1 instanceof InformationExtension, "InformationExtensionのインスタンスであるべき");
    });
  });
  ...
  suite("path property", () => {
    test("正常系 初期状態でundefined", () => {
      // アサート
      assert.strictEqual(InformationExtension.path, undefined, "初期状態ではpathはundefinedであるべき");
    });
    ...
    test("正常系 値を設定できる", () => {
      ...
      // アサート
      assert.strictEqual(InformationExtension.path, testPath, "設定した値が取得できるべき");
