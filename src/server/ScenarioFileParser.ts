...
export class ScenarioFileParser {
  ...
  private processMacroTag(
    ...
    if (!this.pluginTags[macroName]) {
      ctx.isMacro = true;
      const pos = Position.create(data["line"], data["column"]);
      ctx.macroData = new DefineMacroData(
        macroName,
        Location.create(scenarioData.uri, Range.create(pos, pos)),
        absoluteScenarioFilePath,
        ctx.description,
      );
    }
  ...
  private async processLabelTag(
    ...
    const pos = Position.create(await data["line"], await data["column"]);
    this.labelMap
  ...
  private async processTransitionTag(
    ...
    const line = await data["line"];
    const range = Range.create(
      Position.create(line, 0),
      Position.create(line, 0),
    );
    const uri = URI.file(absoluteScenarioFilePath).toString();
    const tagName = await data["name"];
