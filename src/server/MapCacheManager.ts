...
export class MapCacheManager {
  ...
  public async updateScenarioFileMap(filePath: string) {
    ...
    // サーバー側: fs.readFileSync + TextDocument.create で代替
    try {
      const content = fs.readFileSync(filePath, "utf-8");
      const uri = URI.file(filePath).toString();
      const doc = TextDocument.create(uri, "tyrano", 0, content);
      this.scenarioFileMap.set(filePath, doc);
    } catch (error) {
      TyranoLogger.print(
  ...
  public spliceCharacterMapByFilePath(fsPath: string) {
    ...
    const uriStr = URI.file(fsPath).toString();
    const updatedCharacterData = characterMap.filter(
      (value: CharacterData) => {
        return URI.parse(value.location.uri).fsPath !== fsPath;
      },
    );
