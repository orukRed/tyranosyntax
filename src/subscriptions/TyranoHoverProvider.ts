...
export class TyranoHoverProvider {
	...
	constructor() {
		...
		this.jsonTyranoSnippet = JSON.parse(fs.readFileSync(path.join(infoE.path + `${path.sep}Tooltip${path.sep}tyrano.Tooltip.json`), "utf8"));
...
`
	...
	public async provideHover(document: vscode.TextDocument, position: vscode.Position, token: vscode.CancellationToken): Promise<vscode.Hover> {
		...
		if (parameterWordRange) {
			//プロジェクトパス取得
			const projectPath: string = await this.infoWs.getProjectPathByFilePath(document.uri.fsPath);

			//タグ名取得
			const exp = /(\w+)(\s*((\w*)=\"?([a-zA-Z0-9_./\*]*)\"?)*)*/
			const wordRange = document.getWordRangeAtPosition(position, exp);
			const matcher: RegExpMatchArray | null = document.getText(wordRange).match(exp);
			const tag = matcher![1];
