...
/**
 * ワークスペースディレクトリとdata/フォルダの中にある素材情報を管理するシングルトン。
 * 実際のロジックはProjectPathResolver, MapCacheManager, ScriptFileParser, ScenarioFileParserに委譲。
 */
export class InformationWorkSpace {
  ...
  /**
   * マップファイルの初期化。await initializeMaps();の形でコンストラクタの直後に呼んで下さい。
   */
