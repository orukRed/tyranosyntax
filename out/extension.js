...
async function activate(context) {
    const run = async () => {
        await vscode.window.withProgress({
            ...
            infoE.path = context.extensionPath;
            TyranoLogger_1.TyranoLogger.print("TyranoScript_syntax initialize start.");
            try {
                //登録処理
                ...
                infoWs.getTyranoScriptProjectRootPaths().forEach(element => {
                    vscode.window.showInformationMessage(`${element.split(infoWs.pathDelimiter).pop()}の読み込みが完了しました。`);
                });
                vscode.window.showInformationMessage(`TyranoScript_syntaxの初期化が完了しました。`);
            }
            catch (error) {
