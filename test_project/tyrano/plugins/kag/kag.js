tyrano.plugin.kag = {
  ...
  init: function () {
    ...
    if (
      $.isElectron() &&
      window.navigator.userAgent.indexOf("TyranoStudio") == -1
    ) {
  ...
  init_game: function () {
    ...
    if (
      typeof that.variable.sf._system_config_unread_text_skip != "undefined"
    ) {
    ...
    if (
      ...
      this.config["keyFocusOutlineAnim"] !== "none"
    ) {
  ...
  preload: function (src, callbk, options = {}) {
    ...
    if (ext == "wav" || ext == "mp3" || ext == "ogg" || ext == "m4a") {
      ...
      audio_obj.__single_use =
        options.single_use !== undefined ? options.single_use : true;
  ...
  preloadNextVoice: function () {
    ...
    const end_index = Math.min(
      array_tag.length,
      this.kag.ftag.current_order_index + max_search_tag_coung,
    );
    ...
    if (next_chara_ptext_pm) {
      ...
      if (next_chara_voconfig) {
        ...
        const next_voice_storage_path = $.parseStorage(
          next_voice_storage,
          "sound",
        );
  ...
  logTrigger: function (event_name, event_obj) {
    ...
    console.log(
      "%c" + event_name,
      `padding: 4px 6px; border-radius: 4px; background: ${bgcolor}; color: ${color};`,
    );
  ...
  parseKeyframesForWebAnimationAPI: function (name) {
    if (
      !(this.stat.map_keyframe[name] && this.stat.map_keyframe[name].frames)
    ) {
      return null;
    }
  ...
  changeHowlVolume: function (audio_obj, options = {}) {
    ...
    let tag_volume =
      audio_obj.__tag_volume !== undefined ? audio_obj.__tag_volume : 1;

    // 音声再生タグが実行された時点のコンフィグ音量を思い出せ…！ 忘れてたら1だったことにしろ…！
    let config_volume =
      audio_obj.__config_volume !== undefined ? audio_obj.__config_volume : 1;
  ...
  makeFocusable: function (j_elm, tabindex = 0) {
    ...
    j_elm.on("focusin", (e) => {
      ...
      let by_mousedown =
        e.timeStamp - mousedown_timestamp < 10 && mousedown_target === e.target;
      if (by_mousedown) {
